...
function _utils() {
  const data = require("@birman/utils");
...
var _createDefaultConfig = _interopRequireDefault(require("./create-default-config"));
...
const debug = (0, _utils().createDebug)('walrus:test');
...
function _ref() {
  _ref = _asyncToGenerator(function* (args) {
    ...
    if (args.debug) {
      _utils().createDebug.enable('walrus:test');
    }
    ...
    const argsConfig = Object.keys(_args().options).reduce((prev, name) => {
      if (args[name]) prev[name] = name; // Convert alias args into real one
